...
CalibrationData calibration_data[MAX_BASE_STATIONS] = 
{
  ...
  // if channel is switched and station is recalibrated -> only AccelX/Y/Z, mode, unlock count change values -> we do not use them, so changing these lines when new channels are changed is possible
  {},
  {}
};