...
#define differential_manchester_rx_wrap_target 6
#define differential_manchester_rx_wrap 11
...
static inline void differential_manchester_rx_program_init(PIO pio, uint sm, uint offset, uint d_pin, uint e_pin, float div) {
	assert(e_pin == d_pin + 1); // because there is only one offset for pins used fl
    pio_sm_set_consecutive_pindirs(pio, sm, d_pin, 1, false);
	pio_sm_set_consecutive_pindirs(pio, sm, e_pin, 1, false); //fl
	// or try pio_sm_set_consecutive_pindirs(pio, sm, d_pin, 2, false); //fl
    pio_gpio_init(pio, d_pin);
	pio_gpio_init(pio, e_pin); //fl
    pio_sm_config c = differential_manchester_rx_program_get_default_config(offset);
    sm_config_set_in_pins(&c, d_pin); // for WAIT
	// sm_config_set_in_pins(&c, e_pin); // for WAIT // fl we have consecutive pins
    sm_config_set_jmp_pin(&c, d_pin); // for JMP
    sm_config_set_in_shift(&c, true, true, 22); // AUTOPUSH enabled! (is 32 by default) - we need 10 skipped bits+32 other bits -> 44 bits (in 2 chunks are 22 bits per junk)
